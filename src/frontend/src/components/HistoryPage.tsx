...
export function HistoryPage({
  ...
  const [principalInput, setPrincipalInput] = useState(principal);
  ...
  const fetchGlobalTrades = useCallback(async (pg: number) => {
    ...
    try {
      ...
      setErrorGlobal("Failed to load global feed.");
      setGlobalTrades([]);
    } finally {
      ...
      {!principal && (
        <div className="rounded-xl border border-border bg-card p-4 md:p-5 space-y-3">
          <div className="flex items-center gap-3">
            <Input
              data-ocid="history.principal.input"
              placeholder="or enter principal ID manually\u2026"
              value={principalInput}
              onChange={(e) => setPrincipalInput(e.target.value)}
              className="bg-muted/40 border-border"
              onKeyDown={(e) => {
                if (e.key === "Enter" && principalInput.trim())
                  onSetPrincipal(principalInput.trim());
              }}
            />
            <Button
              type="button"
              data-ocid="history.principal.submit_button"
